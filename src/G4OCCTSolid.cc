...
G4ThreeVector G4OCCTSolid::GetPointOnSurface() const {
  return fImpl->kernel.GetPointOnSurface(GetName().c_str());
}
...
G4Polyhedron* G4OCCTSolid::CreatePolyhedron() const {
  ...
  try {
    BRepMesh_IncrementalMesh mesher(fImpl->kernel.Shape(), g4occt::detail::kOCCTRelativeDeflection,
...
void G4OCCTSolid::SetOCCTShape(const TopoDS_Shape& shape) {
  ...
  fImpl->polyhedronGeneration = std::numeric_limits<std::uint64_t>::max();
}
